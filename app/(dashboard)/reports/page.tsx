...
function ReportsHubContent() {
  const { invoices, bookings, enquiries, vehicles } = useFleetStore();
  ...
  const utilRate = Math.round((activeVehicles / (vehicles.length || 1)) * 100);
    ...
    <div className="space-y-6">
      <div className="border-b pb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Financial performance, enquiry conversion funnels, and fleet utilization telemetry.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-l-4 border-l-violet-600 shadow-soft">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-xs font-bold text-muted-foreground uppercase">Fleet Utilization</CardTitle>
            <Car className="h-4 w-4 text-violet-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-extrabold">{utilRate}%</div>
            <p className="text-[11px] text-muted-foreground mt-1">{activeVehicles} active units</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="shadow-soft">
          <CardHeader>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <PieChart className="h-4 w-4 text-primary" /> Vehicle Category Revenue Split
            </CardTitle>
            <CardDescription className="text-xs">
              Percentage share of rental earnings generated by each vehicle class.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-4 space-y-4">
            {[
              { type: 'Innova Crysta / Luxury SUV', share: '52%', color: 'bg-blue-600', val: 1450000 },
              { type: 'Force Traveller (12 & 17 Seater Coach)', share: '28%', color: 'bg-emerald-600', val: 780000 },
              { type: 'Scorpio N / Classic 4WD', share: '12%', color: 'bg-amber-500', val: 340000 },
              { type: 'Maruti Ertiga / Rumion', share: '8%', color: 'bg-violet-600', val: 220000 },
            ].map((cat, idx) => (
